...
$ArchivoDbg ="carga-datos-".$FechaProcesoTS.$HoraProcesoTS.".dbg";
//echo " ArchivoDbg >>$ArchivoDbg<< " ; 
$Hnd = fopen($ArchivoDbg,"w");

$NoCteDbg = $_REQUEST['noCliente'];

fwrite($Hnd, "NoCteDbg >>".$NoCteDbg."<< \n");
...
if($_REQUEST['noCliente']!='')
{
	...
	fwrite($Hnd, "QuerySearch >>".$QuerySearch."<< \n");

	$rec = $db->combo($QuerySearch);
	$count = 0;
	while ($row = $rec->fetch_assoc()) {
		$count++;
		fwrite($Hnd, "count >>".$count."<< \n");
		...
		fwrite($Hnd, "NombreCliente >>".$NombreCliente."<< \n");
		fwrite($Hnd, "NombreClienteMo >>".$NombreClienteMo."<< \n");
		fwrite($Hnd, "DireccionCliente >>".$DireccionCliente."<< \n");
		fwrite($Hnd, "Correo >>".$Correo."<< \n");	
	}
	if (@$count>0){
		fwrite($Hnd, "TypeAutocomplite >>".$TypeAutocomplite."<< \n");
		...
		switch ($TypeAutocomplite) {
			case "OrdenDeTrabajo":
					...
					fwrite($Hnd, "QuerySearchOrden >>".$QuerySearchOrden."<< \n");
					...
					fwrite($Hnd, "QuerySearchOrdenRefac >>".$QuerySearchOrdenRefac."<< \n");
					...
					if($recOrdenDeT->num_rows >=1 ){
						...
						fwrite($Hnd, "QuerySearchOrdenStore >>".$QuerySearchOrdenStore."<< \n");
						...
						$DiffDateTime = $fecha_actual->diff($fecha_entrada);
							if($DiffDateTime->days >=30 ){
								...
								fwrite($Hnd, "Else DiffDateTime->days>>".$DiffDateTime->days."<< \n");
								fwrite($Hnd, "Antes del while(rowRefac = recOrdenDeTRefac->fetch_assoc())  \n");
								$countRefac = 0;
								while($rowRefac = $recOrdenDeTRefac->fetch_assoc()) {
									$countRefac++;
									$Aux1=$rowRefac['ID_REFACCIONES'];
									$Aux2=$rowRefac['PIEZAS_REFACCION'];
									$ID_REFACCIONES[]=$rowRefac['ID_REFACCIONES'];
									$PIEZAS_REFACCION[]=$rowRefac['PIEZAS_REFACCION'];

									fwrite($Hnd,"en el while >> $countRefac <<ID_REFACCIONES >>$Aux1<< PIEZAS_REFACCION >>$Aux2<<\n");

								}
								fwrite($Hnd, "depues  del while(rowRefac = recOrdenDeTRefac->fetch_assoc())  \n");

								if(isset($ID_REFACCIONES))
			...
			case "PolizaGarantia":
					...
					fwrite($Hnd, "getRutaJson >>". $getRutaJson ."<< \n");
