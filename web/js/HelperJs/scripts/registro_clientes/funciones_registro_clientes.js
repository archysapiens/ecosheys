    ...
    $(document).ready(function() {
      ...
      $('#txtnoCliente').blur(function(){
          ...
          var dataString = 'txtnoCliente='+NO_CLIENTE;
            ...
            $.ajax({
                type: "POST",
                url: "../repositorio/validacion_no_cliente.php",
                data: dataString,
                success: function(data) {
                  console.log(data);
					...
					if(data==1){
						...
						$('#btnGuardar_Moral').removeClass('disabled');
						$('#Info').fadeIn('fast').html('<span class="glyphicon glyphicon-ok form-control-feedback"></span><span id="helpBlockNo" class="help-block form-control-feedback" style="width:140px;margin-top:-1px;"> Disponible </span>');
						// $('#Info').fadeIn('fast').html('<div class="form-group has-success has-feedback"><label class="control-label col-xs-3">No. de Cliente:</label><div class="col-xs-8"><input type="text" id="txtnoCliente" name="txtnoCliente" value="'+NO_CLIENTE+'" class="form-control txtnoCliente" placeholder="Introduzca el n&uacute;mero de Cliente" onkeydown="return validarNumeros(event)"><span class="glyphicon glyphicon-ok form-control-feedback"></span></div></div>');
					}
					else{
						$('.has-feedback').removeClass('has-success');
						$('.has-feedback').addClass('has-error');
						$('#btnGuardar_Moral').addClass('disabled');
						// $('#Info').fadeIn('fast').html('<div class="form-group has-error has-feedback"><label class="control-label col-xs-3">No. de Cliente:</label><div class="col-xs-8"><input type="text" id="txtnoCliente" name="txtnoCliente" value="'+NO_CLIENTE+'" class="form-control txtnoCliente" placeholder="Introduzca el n&uacute;mero de Cliente" onkeydown="return validarNumeros(event)"><span class="glyphicon glyphicon-remove form-control-feedback"></span></div></div>');
						$('#Info').fadeIn('fast').html('<span class="glyphicon glyphicon-remove form-control-feedback"></span></span><span id="helpBlockNo" class="help-block form-control-feedback" style="width:140px;margin-top:-1px;"> No disponible </span>');
					}
					//fin de if agregado
